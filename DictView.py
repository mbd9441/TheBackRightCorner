...
class dictview:
    ...
    """
    def __init__(self, window, title, dictlist):
        self.window=window
    ...
    def build_dict_view(self, dictlist):
        ...
        for i in range(0, len(self.dictlist)):
            ...
            for key, value in self.dictlist[i].items():
                if key == self.columns[0]:
                    link=None
                    if key == "Tracking #":
                        link = lambda value=value:self.window.order_page(value, self.dictlist[i])
                    elif key == "Package":
                        link = lambda value=value:self.window.package_page(value, self.orderid, self.dictlist[i])
